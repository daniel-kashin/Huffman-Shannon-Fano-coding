...
int main(int argc, char* argv[])
{
    ...
    if (argc <= 1) { // if no paramether: force to write it
...
encode(bool isHuffman)
{
    ...
    while (fileDoesNotExist) {
        ...
        if (fileDoesNotExist)
            std::cout << "Input file is either empty or does not exist." << std::endl;
    };
    ...
    Util::encodeFile(codes, inputFileName, writer);
